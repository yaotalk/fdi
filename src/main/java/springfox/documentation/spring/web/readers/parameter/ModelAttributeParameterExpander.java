 ...
 * 
 * @author Panxinmiao
 * 修复在ApiModel中，在非基本类型的属性上@ApiModelProperty(hidden = true)不起作用的Bug。
 *
 */
@Primary
@Component
public class ModelAttributeParameterExpander {
  ...
  public List<Parameter> expand(ExpansionContext context) {
    ...
    Iterable<ResolvedField> fields = FluentIterable.from(fieldProvider.in(context.getParamType()))
        .filter(onlyBeanProperties(beanPropNames)).filter(not(hiddenProperties()));
  ...
  /*
   * panxinmiao 判断是否是隐藏属性
   */
  private Predicate<ResolvedField> hiddenProperties() {
    return new Predicate<ResolvedField>() {
      @Override
      public boolean apply(ResolvedField input) {
        ApiModelProperty modelProperty =input.getRawMember().getDeclaredAnnotation(ApiModelProperty.class);
        return modelProperty!=null && modelProperty.hidden();
      }
    };
  }
